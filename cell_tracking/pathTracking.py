...
class TrackedCell():
    ...
    def  __init__(self,center,cell_id):
        ...
        self.prediction = np.asarray(center).reshape(1,2)  # predicted centroids (x,y)
...
class PathTracker():
    ...
    def update(self,image,centers):
        # print(len(centers))
        # print(len(self.tracked_cells))
        # print(len(self.tracked_cells_ids))
...
#                     # If cost is very high then un_assign (delete) the tracker.track
            ...
            # print('len(assignment) + len(un_assigned_detects):',len(assignment) + len(un_assigned_detects))
            # print(assignment)
            ...
            for i in range(len(assignment)):
                ...
                if(assignment[i] != -1):
                    ...
                    self.tracked_cells[i].KF.state,self.tracked_cells[i].prediction = self.tracked_cells[i].KF.correct(np.matrix(cell_centers_in_frame[assignment[i]]).reshape(2, 1),1)
#                     # print(type(tracker.tracks[i].prediction))
# #                     self.tracked_cells[i].positions.append(self.tracked_cells[i].prediction)
                else:
                    self.tracked_cells[i].KF.state, self.tracked_cells[i].prediction = self.tracked_cells[i].KF.correct(np.matrix([[0], [1]]).reshape(2, 1), 0)
...
#                     self.tracked_cells[i].positions.append(self.tracked_cells[i].predictedState)
                self.tracked_cells[i].positions.append(self.tracked_cells[i].prediction[0])
                # if self.frame_id in [2,3,4,5]:
                    
                    # if(self.tracked_cells[i].cell_id  in [31 , 60 , 67 , 71 , 96 , 105 , 111]):
                        #     # print(self.tracked_cells[i].cell_id)
                        # print('i : ->',i,'->>', self.tracked_cells[i].positions)
                        # print(self.tracked_cells[i].prediction)
